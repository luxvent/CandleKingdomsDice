...
private fun throwDice (context: CommandContext<ServerCommandSource>, stringArg: String, throwCount: Int = 1, description: Text = Text.literal("")): Int {
    ...
    for (i in 1..throwCount) {
        ...
        when (stringArg) {
            ...
            "D4" -> {
                result = Random.nextInt(1, 4 + 1)
                max = 4
            }

            "D6" -> {
                result = Random.nextInt(1, 6 + 1)
                max = 6
            }

            "D8" -> {
                result = Random.nextInt(1, 8 + 1)
                max = 8
            }

            "D10" -> {
                result = Random.nextInt(1, 10 + 1)
                max = 10
            }

            "D12" -> {
                result = Random.nextInt(1, 12 + 1)
                max = 12
            }

            "D20" -> {
                result = Random.nextInt(1, 20 + 1)
                max = 20
            }
    ...
    if (resultList.isNotEmpty()) {
        if (throwCount == 1) {
            if (resultList[0] == max) {     // wth? Critical hit from bg3?
        ...
        context.source.sendError(Text.translatable("text.candle-kingdoms-dice.dice_throw.invalid_dice"))
    }
